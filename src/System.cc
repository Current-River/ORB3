...
#include <iomanip>                              // 对cin,cout之类的一些操纵算子
                    ...
                    mbDeactivateLocalizationMode(false)
{
    ...
    cout << "----------加载ORB字典用时："<< time_used.count()<<"秒。"<<endl <<endl;
    ...
    if (mSensor==IMU_STEREO || mSensor==IMU_MONOCULAR)
        mpAtlas->SetInertialSensor();// 根据惯性传感器设置多地图锁？

    //Create Drawers. These are used by the Viewer
    // 这里的帧绘制器和地图绘制器将会被可视化的Viewer所使用
    cout<<"----------创建并初始化绘制帧绘制器和地图绘制器----------"<<endl;
    mpFrameDrawer = new FrameDrawer(mpAtlas);               // 建立一个地图集，画出子地图的帧
    mpMapDrawer = new MapDrawer(mpAtlas, strSettingsFile);  // 建立一个地图集，画出子地图的点云

    //Initialize the Tracking thread
    //(it will live in the main thread of execution, the one that called this constructor)
    cout << "Seq. Name: " << strSequence << endl << endl;

    cout<<"************创建并初始化 Tracking 线程************"<<endl;
    ...
    cout<<"************创建并初始化 Local Mapping 线程************"<<endl;
    ...
    cout<<"************创建并初始化 Loop Closing 线程************"<<endl;
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    // 激活定位模式和解除定位模式
    {
        // 独占锁，主要是为了mbActivateLocalizationMode
        // 和mbDeactivateLocalizationMode不会发生混乱
        unique_lock<mutex> lock(mMutexMode);
        if(mbActivateLocalizationMode)
        {
            // 若定位锁启动，则停止局部建图，只进行跟踪
            mpLocalMapper->RequestStop();
